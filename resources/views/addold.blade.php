        ...
        <table class="table table-bordered">
            ...
            <tbody>    
                @foreach ($records as $record)
                    <tr>
                        <td><input type="hidden" value="{{ $record->date }}" name="record[1][date]"></td>
                        <td>{{ date("j日", strtotime($record->date)) }}</td> 
                        <td><input type="number" value="{{ $record->weight }}" step="0.1"  min="10" max="120" class="form-control"  name="record[1][weight]"></td>                                   
                        <td><input type="number" value="{{ $record->kcal }}" step="1"  min="0" max="4000" class="form-control"  name="record[1][kcal]"></td>
                        <td><input type="number" value="{{ $record->protein }}"  step="1"  min="0" max="500" class="form-control"  name="record[1][protein]"></td>
                        <td><input type="number" value="{{ $record->fat }}" step="1"  min="0" max="500" class="form-control"  name="record[1][fat]" ></td>
                        <td><input type="number" value="{{ $record->carb }}" step="1"  min="0" max="1000" class="form-control"  name="record[1][carb]"></td>
                    </tr>
                @endforeach
